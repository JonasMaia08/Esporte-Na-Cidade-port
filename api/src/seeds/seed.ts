...
export const seedOfAllEntities = async () => {
  ...
  for (const athlete of athletes) {
    ...
    for (const modality of modalities) {
      ...
      enrollment.aproved = faker.number.float({ min: 0, max: 1 }) <= 0.9;
      enrollment.active =
        enrollment.aproved && faker.number.float({ min: 0, max: 1 }) <= 0.9; // Só ativa se aprovada
      ...
      console.log(
        `Inscrição: ${athlete.name} -> ${modality.name} | ` +
          `Status: ${enrollment.aproved ? "Aprovada" : "Recusada"} ` +
